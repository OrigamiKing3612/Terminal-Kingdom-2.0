...
func set_custom_cell_item(position: Vector3i, item: int, orientation: int = 0) -> void:
...
func update_sub_cell( x, y, z ):
	if get_main_cell_item( x, y, z ) == -1:
		sub_gridmap.set_custom_cell_item(Vector3i(x, y, z), -1)
		return
	...
	if meshes.has( mask ):
		...
		sub_gridmap.set_custom_cell_item(Vector3i(x, y, z), mesh, orientation )
	else:
		sub_gridmap.set_custom_cell_item(Vector3i(x, y, z), -1 )
