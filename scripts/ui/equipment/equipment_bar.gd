extends Panel
class_name EquipmentBar

@export var template_item: PackedScene = preload("res://scenes/ui/equipment/template_item.tscn")
@onready var container: Node = $Items
var tool_items: Array[ToolItem]

var selected_item_index: int:
	get: return GameManager.selected_item_index
	set(value): 
		print("EQ: Setting selected_item_index: old value: ", GameManager.selected_item_index)
		GameManager.selected_item_index = value
		print("EQ: Done settings selected_item_index: new value: " , value)

func show_equipment_bar():
	if GameManager.player.items.size() == 0:
		print("EQ: No items")
		return
	if tool_items.size() == 0:
		print("EQ: No tool items")
		return

	clear_children()

	var size = tool_items.size()
	var last2 = (selected_item_index - 2 + size) % size
	var last = (selected_item_index - 1 + size) % size
	var next = (selected_item_index + 1) % size
	var next2 = (selected_item_index + 2) % size

	var indices = [last2, last, selected_item_index, next, next2]

	for display_index in range(indices.size()):
		var item_index = indices[display_index]
		var new_item: EquipmentBarTemplateItem = template_item.instantiate()
		new_item.item = tool_items[item_index]
		new_item.count = tool_items[item_index].durability
		new_item.anchor_left = 1
		new_item.anchor_top = 1
		new_item.anchor_right = 1
		new_item.anchor_bottom = 1

		new_item.position = Vector2(-64, (-128 * (display_index)) - 85)

		container.add_child(new_item)
		new_item.update_display()

		if display_index == 2:  # always select the middle one in the visible list
			new_item.selected()
			#GameManager.selected_gridmap_id = tool_items[item_index].item.gridmap_id
		else:
			new_item.deselected()

func hide_equipment_bar():
	clear_children()

func clear_children():
	for child in container.get_children():
		container.remove_child(child)
		child.free()

func refresh_tool_items():
	tool_items.clear()

	for item in GameManager.player.items:
		if item is ToolItem:
			tool_items.append(item)
	if tool_items.size() > 0:
		show_equipment_bar()


func _on_game_manager_inventory_update() -> void:
	refresh_tool_items()

func _on_tool_next() -> void:
	if not tool_items.size() > 1:
		return
	var index = selected_item_index
	index = (index + 1) % tool_items.size()
	selected_item_index = index
	show_equipment_bar()


func _on_tool_back() -> void:
	if not tool_items.size() > 1:
		return
	var index = selected_item_index
	index = (index - 1 + tool_items.size()) % tool_items.size()
	selected_item_index = index
	show_equipment_bar()
