...
@export var dirt_block: Item # 4
@export var stone_block: Item # 31
@export var gold_ore: Item # 44
@export var iron_ore: Item # 45
@export var coal_ore: Item # 43
@export var clay_block: Item # 64 # mud
	...
	for x in range(int(origin.x), int(origin.x + size.x)):
		for z in range(int(origin.z), int(origin.z + size.z)):
			...
			for y in range(int(origin.y), height):
				...
				var block_id = stone_block.gridmap_id
				...
				if y < size.y - 12:
					...
					if randi() % 100 < 10:
						block_id = clay_block.gridmap_id
